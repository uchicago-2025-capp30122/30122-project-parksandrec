...
def plot_landuse_map(df_path):
    ...
    gdf.drop(
        columns=["OBJECTID", "FIRST_COUNTY", "Shape__Area", "Shape__Length"],
        inplace=True,
    )
    ...
    gdf["landuse_condendenced"] = pd.Categorical(
        gdf["landuse_condendenced"], categories=categories
    )
